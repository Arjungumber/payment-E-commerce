        ...
        {data.map((item, index) => {
          return (
            <div key={item.userId + index}>
                  ...
                  {item.shipping_options.map((shipping, index) => {
                    return (
                      <div key={shipping.shipping_rate} className=" ml-1">
                        Shipping Amount : {shipping.shipping_amount * 20}
                      </div>
                    );
                  })}
                </div>
              ...
              <div className="font-semibold ml-auto w-fit lg:text-lg">Total Amount : {displayINR(item.totalAmount * 82)}</div>
              </div>
            </div>
          );
        })}
